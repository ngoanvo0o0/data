...
exports.AdminConsoleTeamService = void 0;
...
class AdminConsoleTeamService {
    ...
    upsertTeam(userRequest, currentUser) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (userRequest.password) {
                mappedUser.password = yield authentication_service_1.hashSecret(userRequest.password);
            }
            const [user] = yield Users_1.Users.upsert(mappedUser, {
                returning: true
            });
            if (userRequest.id === user.id) {
                histories_service_1.logUpdate(currentUser, user.id, user.name, history_dto_1.HistoryEntityType.Team);
            }
            else {
                histories_service_1.logCreation(currentUser, user.id, user.name, history_dto_1.HistoryEntityType.Team);
            }
            return {
                id: user.id,
                email: user.email,
...
exports.AdminConsoleTeamService = AdminConsoleTeamService;
