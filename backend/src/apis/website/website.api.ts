...
export class WebsiteApi extends Controller {
  ...
  @Get("rao-vat/{slug}")
  public async getRaoVatBySlug(@Path() slug: string): Promise<RaoVatDto> {
  ...
  public async changePassword(
    ...
    const currentUser = (request.authInfo as AuthenticationInfo)?.userId;
    return new AdminConsoleUSerService().changePassword(
      currentUser,
      changePasswordRequest
    );
  }
  ...
  public async updateProfile(
    @Body() updateProfileRequest: any,
    @Request() request: ExRequest
  ): Promise<void> {
    const currentUser = (request.authInfo as AuthenticationInfo)?.userId;
    return new AdminConsoleUSerService().updateUser(
      currentUser,
