...
const buildDialectOptionsFromFile = (path) => {
    ...
    try {
        ...
        exports.error(`Argument -f [--dialect-options-file] '${path}' is not a valid path`);
    }
    try {
...
const buildConfig = (argv) => {
    ...
    const config = {
        ...
        metadata: {
            ...
            ...argv[exports.aliasesMap.CASE] && { case: exports.parseCase(argv[exports.aliasesMap.CASE]) },
...
const buildDialect = (argv) => {
    ...
    switch (argv[exports.aliasesMap.DIALECT]) {
        ...
        case 'mssql':
            ...
            break;
        default:
            exports.error(`Unknown dialect ${argv[exports.aliasesMap.DIALECT]}`);
    }
    return dialect;
};
...
const validateArgs = (argv) => {
    ...
    if (!Dialect_1.Dialect.dialects.has(argv[exports.aliasesMap.DIALECT])) {
        exports.error(`Required argument -D <dialect> must be one of (${Array.from(Dialect_1.Dialect.dialects).join(', ')})`);
    }
    // Validate database
    if (argv[exports.aliasesMap.DIALECT] !== 'sqlite' && !argv[exports.aliasesMap.DATABASE]) {
        exports.error(`Argument -d [database] is required for dialect ${argv[exports.aliasesMap.DIALECT]}`);
    }
    // Validate port
    if (argv[exports.aliasesMap.PORT] && (!Number.isInteger(argv[exports.aliasesMap.PORT]) || argv[exports.aliasesMap.PORT] <= 0)) {
        exports.error(`Argument -p [port] must be a positive integer (${argv[exports.aliasesMap.PORT]})`);
    }
    // Validate case
    if (argv[exports.aliasesMap.CASE]) {
        if (argv[exports.aliasesMap.CASE].includes(':')) {
            ...
            if (!IConfig_1.TransformCases.has(modelCase)) {
                exports.error(`Unknown case '${modelCase}': must be one of (${Array.from(IConfig_1.TransformCases).join(', ').toLowerCase()})`);
            }
            if (!IConfig_1.TransformCases.has(columnCase)) {
                exports.error(`Unknown case '${columnCase}': must be one of (${Array.from(IConfig_1.TransformCases).join(', ').toLowerCase()})`);
            }
        }
        else if (!IConfig_1.TransformCases.has(argv[exports.aliasesMap.CASE].toUpperCase())) {
            exports.error(`Argument -c [case] must be one of (${Array.from(IConfig_1.TransformCases).join(', ').toLowerCase()})`);
        }
    }
    // Validate lint file
    if (argv[exports.aliasesMap.LINT_FILE]) {
        try {
            ...
            exports.error(`Argument -L [lint-file] '${argv[exports.aliasesMap.LINT_FILE]}' is not a valid path`);
        }
    }
    ...
    if (argv[exports.aliasesMap.ASSOCIATIONS_FILE]) {
        try {
            ...
            exports.error(`Argument -a [associations-file] '${argv[exports.aliasesMap.ASSOCIATIONS_FILE]}' is not a valid path`);
        }
    }
