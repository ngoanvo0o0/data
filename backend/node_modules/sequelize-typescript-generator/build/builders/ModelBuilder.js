...
class ModelBuilder extends Builder_1.Builder {
    ...
    static buildColumnPropertyDecl(col, dialect) {
        ...
        return ts.createProperty([
            ...(col.foreignKey ?
                [utils_1.generateArrowDecorator(foreignKeyDecorator, [col.foreignKey.targetModel])]
                : []),
            utils_1.generateObjectLiteralDecorator('Column', buildColumnDecoratorProps(col)),
            ...(col.indices && col.indices.length ?
                col.indices.map(index => utils_1.generateObjectLiteralDecorator('Index', buildIndexDecoratorProps(index)))
                : [])
        ], undefined, col.name, (col.autoIncrement || col.allowNull) ?
    ...
    static buildAssociationPropertyDecl(association) {
        ...
        return ts.createProperty([
            ...(association.sourceKey ?
                [
                    utils_1.generateArrowDecorator(associationName, targetModels, { sourceKey: association.sourceKey })
                ]
                : [
                    utils_1.generateArrowDecorator(associationName, targetModels)
                ]),
    ...
    static buildTableClassDeclaration(tableMetadata, dialect, strict = true) {
        ...
        generatedCode += utils_1.nodeToString(utils_1.generateNamedImports([
            'Model',
            'Table',
            'Column',
            'DataType',
        ...
        [...importModels].forEach(modelName => {
            generatedCode += utils_1.nodeToString(utils_1.generateNamedImports([modelName], `./${modelName}`));
            generatedCode += '\n';
        });
        const attributesInterfaceName = `${name}Attributes`;
        if (strict) {
            generatedCode += '\n';
            const attributesInterface = ts.createInterfaceDeclaration(undefined, undefined, ts.createIdentifier(attributesInterfaceName), undefined, undefined, [
                ...(Object.values(columns).map(c => {
                    var _a;
                    return ts.createPropertySignature(undefined, ts.createIdentifier(c.name), c.autoIncrement || c.allowNull ? ts.createToken(ts.SyntaxKind.QuestionToken) : undefined, ts.createTypeReferenceNode((_a = dialect.mapDbTypeToJs(c.type)) !== null && _a !== void 0 ? _a : 'any', undefined));
                }))
            ]);
            generatedCode += utils_1.nodeToString(attributesInterface);
            generatedCode += '\n';
        }
        const classDecl = ts.createClassDeclaration([
            // @Table decorator
            utils_1.generateObjectLiteralDecorator('Table', {
                tableName: tableName,
        ...
        generatedCode += '\n';
        generatedCode += utils_1.nodeToString(classDecl);
    ...
    static buildIndexExports(tablesMetadata) {
        return Object.values(tablesMetadata)
            .map(t => utils_1.nodeToString(utils_1.generateIndexExport(t.name)))
            .join('\n');
    }
    /**
    ...
    async build() {
        ...
        try {
            ...
            linter.lintFiles([path_1.default.join(outDir, '*.ts')]);
        }
        catch (err) {
