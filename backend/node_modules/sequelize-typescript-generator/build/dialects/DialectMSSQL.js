    ...
    async fetchColumnsMetadata(connection, config, table) {
        ...
        for (const column of columns) {
            ...
            if (!this.mapDbTypeToSequelize(column.DATA_TYPE)) {
                utils_1.warnUnknownMappingForDataType(column.DATA_TYPE);
            }
            ...
            switch (column.DATA_TYPE) {
                ...
                case 'double':
                    columnMetadata.dataType +=
                        utils_1.generatePrecisionSignature(column.NUMERIC_PRECISION, column.NUMERIC_SCALE);
                    break;
                case 'datetime2':
                    columnMetadata.dataType += utils_1.generatePrecisionSignature(column.DATETIME_PRECISION);
                    break;
                case 'char':
                case 'nchar':
                case 'varchar':
                case 'nvarchar':
                    columnMetadata.dataType += utils_1.generatePrecisionSignature(column.CHARACTER_MAXIMUM_LENGTH);
                    break;
            }
